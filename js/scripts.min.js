 ...
 */
 var countingdata = 0;
 let provider, signer;
 const nftAddress = "0x8C85Bcc27541De926324Aa2E39b73718C01B9fc1"
 ...
 document.getElementById("counting").innerText = countingdata;
   
 //creation of increment function
 function increment() {
   if(countingdata < 40){
    countingdata = countingdata + 1;
     document.getElementById("counting").innerText = countingdata;
     document.getElementById("totalamount").innerHTML = `${countingdata * 0.5} BNB`;
 ...
 async function connectMeta(){
   try{
    ...
    document.getElementsByClassName("connect_wallet")[0].innerText = `${walletAddress.toString().slice(0, 6)}....`;
  } catch (error) {
    console.log(error);
  }
}
...
async function mintNow (){
  try{
    ...
    const signedNft = await nft.connect(signer);
    console.log ("Counting data", (("0.5")))
    let amountIn = ethers.utils.parseUnits("1", 'ether').mul(countingdata.toString()).div("2");
    const gas = await signedNft.mintApe(countingdata, {value: amountIn});
  } catch (error) {
    console.log (error);
    let message;
    if (error.reason) message = "Minting did not start yet! Keep trying :)";
...
async function totalMinted (){
  try{
    ...
    const signedNft = await nft.connect(signer);
    let minted = await signedNft.totalSupply();
    console.log ("Total minted: " + minted);
    document.getElementsByClassName("totalminted")[0].innerText = `${minted}/40 Minted`
  } catch(err){
    alert (err);
  }
}
